...
##READ IN DATA
setwd("~/Desktop/Abacus_gesture/*first_paper/abacus_gesture_analyses/Study2/experts/finaldata") # SET WORKING DIRECTORY TO FOLDER CONTAINING FINAL DATA (OUTPUTTED FROM PREPROCESSING SCRIPT)
...
plot <- barplot(threshBar,ylim=c(0,7), ylab="Mean Threshold Level", main=c("Mean Level by Condition"), col=c("Black","dark gray","light gray","white"))
ses <- tapply(data.bysub$threshold, data.bysub$condition, FUN=std.error)
plotCI(plot, threshBar, ses, add=T, col=c("gray"))

#Graph 3: overall threshold graph by trial
par(wd=2, mfrow=c(1,2), cex=1.3)
means <- tapply(data.bytrial$threshold, list(data.bytrial$condition, data.bytrial$trial),mean,na.rm=T)
plot(means[1,1:20],type="l",col="blue", xlab=c("Trial"), ylab=c("Level"),main=c("MA Experts:\nAverage Performance Over First 20 Trials"), lty=1, ylim=c(2,8))
